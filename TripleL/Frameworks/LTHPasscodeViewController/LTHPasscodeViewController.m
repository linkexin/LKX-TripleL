...
static NSInteger const kMaxNumberOfAllowedFailedAttempts = 3;
...
- (void)viewDidLoad {
	...
	_failedAttemptLabel.text = @"第 1 次验证失败";
	...
	_enterPasscodeLabel.text = _isUserChangingPasscode ? NSLocalizedString(@"请输入你的旧密码", @"") : NSLocalizedString(@"请输入密码", @"");
...
- (void)showForEnablingPasscodeInViewController:(UIViewController *)viewController {
	...
	self.title = NSLocalizedString(@"开启密码验证", @"");
}
...
- (void)showForTurningOffPasscodeInViewController:(UIViewController *)viewController {
	...
	[self prepareNavigationControllerWithController: viewController];
	self.title = NSLocalizedString(@"关闭密码验证", @"");
}
...
- (void)resetUI {
	...
	if (_isUserConfirmingPasscode) {
		if (_isUserEnablingPasscode) _enterPasscodeLabel.text = NSLocalizedString(@"请再次输入密码", @"");
		else if (_isUserChangingPasscode) _enterPasscodeLabel.text = NSLocalizedString(@"请再次输入你的新密码", @"");
	}
	else if (_isUserBeingAskedForNewPasscode) {
		if (_isUserEnablingPasscode || _isUserChangingPasscode) {
			_enterPasscodeLabel.text = NSLocalizedString(@"请输入你的新密码", @"");
		}
	}
	else _enterPasscodeLabel.text = NSLocalizedString(@"输入密码", @"");
...
- (void)resetUIForReEnteringNewPasscode {
	...
	NSString *savedPasscode = [SFHFKeychainUtils getPasswordForUsername: [[NSUserDefaults standardUserDefaults] objectForKey:kKeychainUsername]
														 andServiceName: [[NSUserDefaults standardUserDefaults] objectForKey:kKeychainServiceName]
																  error: nil];
	_enterPasscodeLabel.text = savedPasscode.length == 0 ? NSLocalizedString(@"输入密码", @"") : NSLocalizedString(@"输入新密码", @"");
	
	_failedAttemptLabel.hidden = NO;
	_failedAttemptLabel.text = NSLocalizedString(@"两次密码不匹配，请重新输入！", @"");
...
//													  userInfo: nil];
	...
	if (_failedAttempts == 1) _failedAttemptLabel.text = NSLocalizedString(@"第 1 次验证失败", @"");
	else {
		_failedAttemptLabel.text = [NSString stringWithFormat: NSLocalizedString(@"第 %i 次验证失败", @""), _failedAttempts];
	}
