...
- (void)performZoomingAnimationWithTransitionContext:(id <UIViewControllerContextTransitioning>)transitionContext {
    ...
    if (![NYTOperatingSystemCompatibilityUtility isiOS8OrGreater] && orientation != UIInterfaceOrientationMaskPortrait) {
        // Correct the endingView and startingView's initial transforms
        endingViewForAnimation.transform = CGAffineTransformConcat([self transformForOrientation:orientation], endingViewForAnimation.transform);
        startingViewForAnimation.transform = CGAffineTransformConcat([self transformForOrientation:orientation], startingViewForAnimation.transform);
        
        // Correct the endingView's final transform
        finalEndingViewTransform = CGAffineTransformConcat([self transformForOrientation:orientation], self.endingView.transform);
    }
    else {
