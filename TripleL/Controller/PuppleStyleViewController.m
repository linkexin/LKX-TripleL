...
@interface PuppleStyleViewController ()
{
    NSArray *data;
}
...
@implementation PuppleStyleViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    [self setNavTitle:@"气泡设置"];
    
    UIView *view = [[UIView alloc] init];
    [view setBackgroundColor:[UIColor clearColor]];
    [self.tableView setTableFooterView:view];

    data = [AppConfig getPuppleStyleArray];
}
...
{
    if (((NSString *)[[NSUserDefaults standardUserDefaults] objectForKey:@"puppleStyle"]).intValue == indexPath.row - 1) {
        return;
    }
    SCLAlertView *alertView = [[SCLAlertView alloc] init];
    [alertView addButton:@"确定并重启" actionBlock:^{
        [[NSUserDefaults standardUserDefaults] setObject:[NSString stringWithFormat:@"%d", (int)(indexPath.row - 1)] forKey:@"puppleStyle"];
    ...
    [alertView showInfo:self.tabBarController title:@"重要提示" subTitle:@"修改气泡设置需要重启应用哦" closeButtonTitle:@"取消" duration:0];
