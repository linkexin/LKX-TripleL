...
- (void) setupView:(UIImage*)backgroundImage avatarImage:(UIImage *)avatarImage titleString:(NSString *)titleString subtitleString:(NSString *)subtitleString buttonTitle:(NSString *)buttonTitle scrollHeight:(CGFloat)height type:(MBType)type
{
    ...
    // Header
    self.header = [[UIView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y, self.frame.size.width, self.frame.size.height * 0.32)];
    ...
    if (type == MBTable) {
        ...
        // TableView Header
        self.tableView.tableHeaderView = [[UIView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y, self.frame.size.width, self.header.frame.size.height + 70)];
    ...
    self.avatarImage = [[UIImageView alloc] initWithFrame:CGRectMake(30, self.header.frame.origin.y + self.header.frame.size.height - 40, 80, 80)];
    ...
    UILabel * titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.avatarImage.frame.origin.x, self.avatarImage.frame.origin.y + self.avatarImage.frame.size.height, 250, 25)];

    titleLabel.text = titleString;
    
    UILabel * subtitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.avatarImage.frame.origin.x, 177, 250, 25)];
    ...
    if (buttonTitle.length > 0) {
        headerButton = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width - 100, self.header.frame.origin.y + self.header.frame.size.height + 15, 80, 35)];
    ...
    UIButton *backButton = [[UIButton alloc]initWithFrame:CGRectMake(0, 10, 100, 50)];
...
- (void) animationForScroll:(CGFloat) offset {
    CGFloat  offset_HeaderStop = self.frame.size.height * 0.15;
    ...
    if (offset < 0) {
        ...
        CGFloat headerSizevariation = ((self.header.bounds.size.height * (1.0 + headerScaleFactor)) - self.header.bounds.size.height) / 2;
