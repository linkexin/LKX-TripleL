...
@interface FontStyleViewController ()
{
    NSArray *data;
}
@end
...
- (void)viewDidLoad {
    ...
    [self setNavTitle:@"字体方案"];
    
    UIView *view = [[UIView alloc] init];
    [view setBackgroundColor:[UIColor clearColor]];
    [self.tableView setTableFooterView:view];
    
    data = [AppConfig getFontStyleArray];
}
...
{
    if (((NSString *)[[NSUserDefaults standardUserDefaults] objectForKey:@"fontStyle"]).intValue == indexPath.row - 1) {
        return;
    }
    SCLAlertView *alertView = [[SCLAlertView alloc] init];
    [alertView addButton:@"确定并重启" actionBlock:^{
        [[NSUserDefaults standardUserDefaults] setObject:[NSString stringWithFormat:@"%d", (int)(indexPath.row - 1)] forKey:@"fontStyle"];
    ...
    [alertView showInfo:self.tabBarController title:@"重要提示" subTitle:@"修改字体方案需要重启应用哦" closeButtonTitle:@"取消" duration:0];
